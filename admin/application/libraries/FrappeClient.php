...
class FrappeClient {
	...
	const CONF_FILE = 'FrappeConfig.php';
	...
	function get_data($key){
		...
		$sql = "SELECT * FROM ".$dbprefix."erpnext_sync_config WHERE config_key = ?";
	...
	function __construct(
			...
			$conf = array(
					...
					'report_url' => $erpnext_url .'/api/method/frappe.desk.query_report.run',
			...
			switch($method){
				...
				case 'SEARCH':
						...
						if($params['limit_start']){
							$query['limit_start'] = $params['limit_start'];
						}
						if($params['limit_page_length']){
							$query['limit_page_length'] = $params['limit_page_length'];
						}
						$url = $this->_api_url.$params['doctype'];
						if($query){
							$url .= '?';
					...
					case 'SEARCH_REPORT':
					    $query = array();
							...
							$url = $this->_report_url."?report_name=".$params['report_name'];
