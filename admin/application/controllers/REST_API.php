...
class REST_API extends CI_Controller {
	...
	public function cart(){
        ...
        if($cart) {
            ...
            return json_encode(['status' => 'success', 'data' => $cart]);
        } else {
            return json_encode(['status' => 'fail', 'data' => null]);
        }
	}
	public function cartClear(){
		$username =  $_POST['username'];
		$cart_id = $this->cart->clear_user_cart($username);
	...
	public function auth(){
		...
		$username = $data_array['user']['username'];
		$password = $data_array['user']['password'];
		try{
			$erpnext = new FrappeClient($username,$password);
//			print_r($erpnext->body);die;
			if ($erpnext->body->full_name) {
			    echo json_encode(['status' => 'success', 'data' => $erpnext->body]);
            } else {
                echo json_encode(['status' => 'fail', 'data' => null]);
            }
//			echo $erpnext->body->full_name;
		} catch (Exception $e) {
//		    echo "Auth fail";
